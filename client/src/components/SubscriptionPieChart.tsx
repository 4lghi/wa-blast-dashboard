...
const SubscriptionPieChart = ({ data, total, compact = false }: SubscriptionPieChartProps) => {
    ...
    <div className={`pie-chart-container ${compact ? "compact" : ""}`}>
      ...
      {compact && (
        <div className="compact-chart-summary">
          ...
          <div className="compact-summary-item">
            <span className="compact-summary-label">Langganan:</span>
            <span className="compact-summary-value">
              {total > 0 ? Math.round(((data.find((d) => d.name === "Berlangganan")?.value || 0) / total) * 100) : 0}%
            </span>
          </div>
        </div>
      )}
